...
<section class="revision-container" aria-labelledby="revision-heading">
    <header class="revision-header">
        <h2 id="revision-heading">Algebra Practice: Straight Line Equations</h2>
...
<!-- 1. Load Generators -->
<script src="{{ '/assets/js/nbs4107a/algebra-lines.js' | relative_url }}"></script>
<!-- 2. Load Core Logic -->
<script src="{{ '/assets/js/algebra-revision-core.js' | relative_url }}"></script>

<script>
    document.addEventListener('DOMContentLoaded', () => {
        if (window.initAlgebraRevisionTest && window.lineGenerators) {
            window.initAlgebraRevisionTest([
                { generator: window.lineGenerators.type1, count: 2 },
                { generator: window.lineGenerators.type7, count: 1 },
                { generator: window.lineGenerators.type2, count: 1 },
                { generator: window.lineGenerators.type3, count: 2 },
                { generator: window.lineGenerators.type4, count: 1 },
                { generator: window.lineGenerators.type5, count: 1 },
                { generator: window.lineGenerators.type6, count: 2 }
            ]);
        } else {
