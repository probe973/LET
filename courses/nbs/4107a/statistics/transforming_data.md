...
<section class="revision-container" aria-labelledby="revision-heading">
    <header class="revision-header">
        <h2 id="revision-heading">Statistics Practice: Transforming Data - Effect on Average and Dispersion Measures</h2>
    ...
    document.addEventListener('DOMContentLoaded', () => {
        if (window.initAlgebraRevisionTest && window.averagesGenerators) {
            window.initAlgebraRevisionTest([
                { generator: window.averagesGenerators.type5, count: 1 },
                { generator: window.averagesGenerators.type6, count: 1 },
                { generator: window.averagesGenerators.type7, count: 1 },
                { generator: window.averagesGenerators.type8, count: 1 }
            ]);
        }
    });
</script>
